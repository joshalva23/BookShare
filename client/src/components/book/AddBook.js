...
function AddBook(props) {
  ...
  //   axios.post("http://localhost:4000/books/add/", formData)
  //     .then(res => {
  //       if (res.status >= 200 && res.status < 300) {
  //         history.replace(`/users/profile/${props.auth.user.id}`);
  //       } else {
  //         // Handle potential error response from the server (optional)
  ...
  const submitAddBookForm = (event) => {
    ...
    axios.post("http://localhost:4000/books/add", formData)
      ...
      .catch(error => {
        console.error('Axios error:', error.response); // Log the entire error object
      });
  };
